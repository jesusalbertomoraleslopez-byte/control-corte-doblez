...
def save_production_plan(of_number, proyecto, programador, fecha, df_nidos, df_piezas,
    ...
    default_ruta = "Ingenieria, Corte, Rebabeo, Doblez, Liberado, Empaque, Almacen PT"
...
def get_dashboard_stats(of_list=None):
    ...
    # Progreso por area
    areas = ["Ingenieria", "Corte", "Rebabeo", "Doblez", "Liberado", "Empaque", "Almacen PT"]
    for area in areas:
        if area == "Ingenieria":
...
def check_and_seed_personal_areas(cursor):
    ...
    default_mapping = {
        ...
        "📦 Embarque": ["Empaque", "Liberado", "Almacen PT"],
        "👑 Dirección": ["Liberado"]
    }

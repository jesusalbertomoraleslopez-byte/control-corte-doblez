...
def view_dashboard_global():
    ...
    # 1. Estado de Producciones (Dona)
    of_stats = []
    for _, row in df_ofs.iterrows():
        of = row['of_number']
        tot = df_totales[df_totales['of_number'] == of]['total_piezas'].sum()
        av = df_avances[df_avances['of_number'] == of]['cantidad'].sum()
        
        estado = "Pendiente"
        if av > 0: estado = "En Proceso"
        if av >= (tot * 4) and tot > 0: estado = "Completada" # Estimando 4 procesos por pieza min
        of_stats.append({"OF": of, "Estado": estado})
    
    df_of_estado = pd.DataFrame(of_stats)

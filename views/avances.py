...
def view_avances():
    ...
    # Mostrar historial de movimientos de esta área (solo post-corte)
    if is_post_corte:
        ...
        if not df_movimientos.empty:
            ...
            st.markdown("👇 *Si necesitas corregir/eliminar un movimiento, dirígete a la sección 3.3 (Corrección).*")
            st.dataframe(df_movimientos, use_container_width=True, hide_index=True)
    else:
        # Mostrar historial de rechazos globales de la OF (para otras áreas)
        ...
        if not df_rechazos_hist.empty:
            ...
            st.dataframe(df_rechazos_hist, use_container_width=True, hide_index=True)
